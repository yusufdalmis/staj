...
export default async function UserRaporDetayPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  // Prevent users from viewing other people's reports
  ...
  return (
    <div className="space-y-6 animate-in fade-in pt-16 md:pt-4 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="overflow-x-auto mt-2 rounded-xl border border-brand-dark/10">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-brand-light/50 border-b border-brand-dark/10">
                          <tr>
                            <th className="px-4 py-2 text-brand-dark">Bileşen Kategorisi</th>
                            ...
                            <th className="px-4 py-2 text-brand-dark">Gelecek Dönem Planı</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-brand-dark/5">
                          {report.annualDetails.components.length === 0 ? (
                            <tr><td colSpan={6} className="px-4 py-4 text-center text-brand-dark/50">Kayıtlı bileşen yok.</td></tr>
                          ) : (
                            report.annualDetails.components.map((comp: any) => (
                              <tr key={comp.id} className="hover:bg-brand-light/10">
                                ...
                                <td className="px-4 py-3 text-brand-dark">{comp.componentName || "-"}</td>
                                ...
                                <td className="px-4 py-3 text-brand-dark">{comp.nextPeriodPlan || "-"}</td>
                              </tr>
                            ))
                          )}
                        </tbody>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5">
            ...
            {report.activities.length === 0 ? (
              ...
              <div className="space-y-6">
                {report.activities.map((act: any, index: number) => (
                      ...
                      {act.stakeholders && act.stakeholders.length > 0 && (
                        <div className="bg-white p-3 rounded-xl border border-brand-dark/5 md:col-span-2">
                          ...
                          <div className="flex flex-wrap gap-2 mt-1">
                            {act.stakeholders.map((s: string, idx: number) => (
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5">
            <h3 className="font-bold text-brand-dark mb-4">Faaliyet Bilgileri</h3>
