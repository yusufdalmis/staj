...
export default function EditReportClient({ report, lists }: { report: any, lists: any }) {
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormValues>({
    defaultValues: {
      ...
      sopDurationYear: null,
      sopDurationMonth: null,
  ...
  useEffect(() => {
    ...
    if (report.annualDetails?.sopDuration) {
      const yearMatch = report.annualDetails.sopDuration.match(/(\d+)\s*Yıl/i);
      if (yearMatch) initialYear = parseInt(yearMatch[1]);
      const monthMatch = report.annualDetails.sopDuration.match(/(\d+)\s*Ay/i);
      if (monthMatch) initialMonth = parseInt(monthMatch[1]);
    }
    if (initialYear !== null) setValue("sopDurationYear", initialYear);
    if (initialMonth !== null) setValue("sopDurationMonth", initialMonth);
  }, [report, setValue])

  useEffect(() => {
    if (session?.user?.unit && lists.UNITS?.includes(session.user.unit)) {
      setValue("unit", session.user.unit)
    }
  }, [session, setValue, lists])
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const payload = {
        ...
        unit: finalUnit,
        activities: !data.isAnnual ? data.activities.map(act => ({
            title: act.title,
            ...
            photo1: act.photo1,
            photo2: act.photo2,
            stakeholders: Array.isArray(act.stakeholders) ? act.stakeholders : (act.stakeholders ? String(act.stakeholders).split(",").map((s: string) => s.trim()).filter(Boolean) : [])
        })) : undefined
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5">
          <div className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-brand-light/50 border border-brand-dark/10 mb-6 gap-4">
            ...
            <div className="flex bg-brand-dark/10 p-1 rounded-xl w-full md:w-auto min-w-[300px]">
              <button 
                type="button" 
                onClick={() => setValue("isAnnual", false)}
                className={`flex-1 px-4 py-2 text-sm font-bold rounded-lg transition-all ${!isAnnual ? 'bg-white shadow-sm text-brand-primary' : 'text-brand-dark/70 hover:text-brand-dark'}`}
              >
                Haftalık Faaliyet
              </button>
              <button 
                type="button" 
                onClick={() => setValue("isAnnual", true)}
                className={`flex-1 px-4 py-2 text-sm font-bold rounded-lg transition-all ${isAnnual ? 'bg-white shadow-sm text-brand-primary' : 'text-brand-dark/70 hover:text-brand-dark'}`}
              >
                Yıllık Ara Rapor
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                {(lists.UNITS || []).map((u: string) => <option key={u} value={u}>{u}</option>)}
              </select>
            ...
            {unit === "Yatırım Destek Ofisi Faaliyetleri (YDO)" && (
              <div>
                ...
                >
                  <option value="">Seçiniz...</option>
                  {(lists.SUB_UNITS || []).map((su: string) => <option key={su} value={su}>{su}</option>)}
                </select>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-primary/20 space-y-8 animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <select 
                  {...register("sopName", { required: isAnnual ? "SOP seçimi zorunludur" : false })}
                  className="w-full px-4 py-2.5 rounded-xl bg-white border border-brand-dark/20 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 outline-none transition-all font-medium text-brand-dark"
                >
                  <option value="">Seçiniz...</option>
                  {(lists.SOPS || []).map((s: string) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
                  ...
                  <div key={field.id} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border border-brand-dark/10 rounded-lg relative">
                    ...
                    <div>
                      ...
                      <select {...register(`components.${index}.name` as const)} className="w-full p-2 border border-brand-dark/20 rounded-lg text-sm text-brand-dark outline-none focus:border-brand-primary">
                        <option value="">Seçiniz...</option>
                        {(lists.COMPONENTS || []).map((c: string) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div>
                  ...
                  <div key={field.id} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border border-brand-dark/10 rounded-lg relative">
                    ...
                    <div>
                      ...
                      <select {...register(`outputIndicators.${index}.componentCode` as const)} className="w-full p-2 border border-brand-dark/20 rounded-lg text-sm text-brand-dark outline-none focus:border-brand-primary">
                        <option value="">Seçiniz...</option>
                        {(lists.PROGRAM_TYPES?.length ? lists.PROGRAM_TYPES : COMPONENT_CODES).map((code: string) => <option key={code} value={code}>{code}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-brand-dark mb-1">Birim</label>
              ...
              <div key={field.id} className="p-5 border border-brand-dark/10 bg-brand-light/10 rounded-2xl relative space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="">Seçiniz...</option>
                      {(lists.PROGRAM_TYPES?.length ? lists.PROGRAM_TYPES : COMPONENT_CODES).map((pt: string) => <option key={pt} value={pt}>{pt}</option>)}
                    </select>
                  </div>

                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-brand-dark mb-2">Temas Edilen Kurum/Paydaşlar <span className="text-brand-primary">*</span></label>
                    <div className="p-3 bg-white border border-brand-dark/20 rounded-xl focus-within:border-brand-primary transition-colors min-h-[50px]">
                      ...
                      >
                        ...
                        {(lists.CONTACTED_INSTITUTIONS || [])
                          .filter((inst: string) => !(activities[index]?.stakeholders || []).includes(inst))
                          .map((inst: string) => <option key={inst} value={inst}>{inst}</option>)}
                      </select>
                    </div>
        ...
        <div className="flex justify-end pt-4 pb-12">
          <button
            ...
            className="flex items-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-brand-primary to-brand-secondary text-white font-bold shadow-lg shadow-brand-primary/25 hover:shadow-brand-primary/40 transform hover:-translate-y-0.5 transition-all active:translate-y-0 disabled:opacity-70 disabled:cursor-not-allowed"
          >
