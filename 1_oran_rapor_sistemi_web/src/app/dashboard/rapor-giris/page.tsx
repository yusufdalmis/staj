...
function RaporGirisContent() {
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormValues>({
    defaultValues: {
      ...
      components: [{ name: "", componentName: "", status: "", delayReason: "", progress: "", nextPeriodPlan: "" }],
      resultIndicators: [],
      outputIndicators: [],
      milestones: [],
      evaluations: [],
  ...
  useEffect(() => {
    if (typeParam === "ANNUAL" || typeParam === "WORK_PROGRAM" || typeParam === "WEEKLY") {
      setValue("reportType", typeParam as "WEEKLY" | "ANNUAL" | "WORK_PROGRAM")
    }
  }, [typeParam, setValue])
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then(data => {
        setLists(data)
        if (session?.user?.unit && data.UNITS?.includes(session.user.unit)) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-16 md:pt-4 pb-20">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-brand-light/50 border border-brand-dark/10 gap-4">
            <div>
            ...
            <div className="flex bg-brand-dark/10 p-1 rounded-xl w-full md:w-auto overflow-x-auto min-w-0">
              <button 
                type="button" 
                onClick={() => setValue("reportType", "WEEKLY")}
                className={`px-4 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${reportType === "WEEKLY" ? 'bg-white shadow-sm text-brand-primary' : 'text-brand-dark/70 hover:text-brand-dark'}`}
              >
                Haftalık Faaliyet
              </button>
              <button 
                ...
                className={`px-4 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${reportType === "ANNUAL" ? 'bg-white shadow-sm text-brand-primary' : 'text-brand-dark/70 hover:text-brand-dark'}`}
              >
                Yıllık Ara Rapor
              </button>
              <button 
                type="button" 
                onClick={() => setValue("reportType", "WORK_PROGRAM")}
                className={`px-4 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${reportType === "WORK_PROGRAM" ? 'bg-white shadow-sm text-brand-primary' : 'text-brand-dark/70 hover:text-brand-dark'}`}
              >
                Çalışma Programı
              </button>
            </div>
      ...
      {reportType === "WORK_PROGRAM" ? (
        <WorkProgramEditor programId="yeni" hideHeader={true} />
      ) : (
                    ...
                    <div className="p-3 bg-white border border-brand-dark/20 rounded-xl focus-within:border-brand-primary transition-colors min-h-[50px]">
                      ...
                      >
                        ...
                        {Array.from(new Set([...(lists.UNITS || []), ...(lists.SUB_UNITS || [])]))
                          .filter(u => !(typeof activities[index]?.nextStep === "string" ? activities[index].nextStep.split(",").map((s: string) => s.trim()).filter(Boolean) : []).includes(u))
...
export default function RaporGirisPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center"><Loader2 className="animate-spin inline mr-2 text-brand-primary" />Yükleniyor...</div>}>
      <RaporGirisContent />
    </Suspense>
  )
}
