  ...
  if (!program) {
    return (
      <div className="bg-white/80 p-12 rounded-2xl border border-brand-dark/5 shadow-sm text-center">
        ...
        <Link href="/dashboard/calisma-programlari" className="inline-flex items-center gap-2 bg-brand-primary text-white px-6 py-2.5 rounded-xl font-medium hover:bg-brand-primary/90 transition-all">
          <ArrowLeft size={20} />
          Listeye Dön
        </Link>
      </div>
    ...
    <div className="space-y-6 animate-in fade-in pt-16 md:pt-4 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-gradient-to-r from-brand-primary/5 via-white/80 to-white/80 backdrop-blur-xl p-6 rounded-3xl border border-white/50 shadow-sm sticky top-20 z-30">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/calisma-programlari" className="w-10 h-10 flex items-center justify-center bg-white rounded-xl shadow-sm hover:bg-brand-primary hover:text-white text-brand-dark/70 transition-all">
            <ArrowLeft size={20} />
          </Link>
          <div>
        ...
        <div className="w-full md:w-auto flex flex-col md:flex-row items-center gap-3">
          ...
          {program && session?.user && (session.user.role === 'ADMIN' || session.user.role === 'SUPER_ADMIN' || session.user.id === program.userId) && (
            <Link 
              href={`/dashboard/program-duzenle/${program.id}`}
