...
import { FileText, Calendar, Search, Download, FileType2, FileCode2 } from "lucide-react"
        ...
        {selectedIds.length > 0 && (
          <div className="flex flex-wrap gap-3 bg-white/60 p-2.5 rounded-2xl border border-brand-primary/20 shadow-inner">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {filteredReports.filter(r => selectedIds.includes(r.id)).every(r => r.isAnnual) && (
                <>
                  <div className="w-px bg-brand-primary/20 mx-1 hidden sm:block"></div>
                ...
                {filteredReports.map((report) => (
                  <tr key={report.id} className="hover:bg-brand-light/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right flex items-center justify-end gap-3">
                      <Link 
                        href={`/dashboard/admin/raporlar/${report.id}`}
