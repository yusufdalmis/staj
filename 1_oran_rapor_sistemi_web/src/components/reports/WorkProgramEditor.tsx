...
import { ArrowLeft, Save, Plus, Trash2, Download, X, Loader2 } from "lucide-react"
import { exportWorkProgramAsDesigned } from "@/lib/exportWordWorkProgram"
import { useDialog } from "@/components/DialogProvider"

export default function WorkProgramEditor({ programId, hideHeader = false }: { programId: string, hideHeader?: boolean }) {
  ...
  const [activities, setActivities] = useState<any[]>(isNew ? [{ id: "", name: "", relatedGoal: "", responsibleUnit: "", supportUnit: "", stakeholders: [], budgetCode: "", budgets: [{ name: "", code: "", amount: "" }], plannedMonths: [], performanceIndicator: "", resultIndicator: "", measurementUnit: "", target: "", verificationSource: "" }] : [])
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        ...
        router.push(`/dashboard/program-detay/${data.id}`)
      } else {
        ...
        router.push(`/dashboard/program-detay/${id}`)
      }
    } catch (error) {
      ...
      {!hideHeader && (
        <div className="flex justify-between items-center bg-white/60 p-6 rounded-2xl backdrop-blur-xl border border-white/50 shadow-sm">
          <div className="flex items-center gap-4">
            <Link href="/dashboard/calisma-programlari" className="p-2 bg-white rounded-lg shadow-sm hover:bg-brand-light text-brand-dark/70 transition-colors">
              <ArrowLeft size={20} />
            </Link>
            <div>
